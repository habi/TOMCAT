...
parser.add_option('-a', '--angle', dest='angle',
                  ...
                  default=180 - 0.001)
parser.add_option('-i', '--initialangle', dest='initial_angle',
                  help='Initial Angle', metavar='0', default=0)
parser.add_option('-l', '--slice', dest='slice',
                  help='Slice number to take from each scan', metavar='1000',
                  default=1000)
parser.add_option('-m', '--multiplefolders', dest='multiple',
                  help='Measurements are in seperate folders',
...
def generate_slices(folder_prefix, sliceNumber):
    ...
    try:
        if multiple:
            ...
            scanNames = map(lambda x: x.split('/')[-3],
    ...
    outNameFn = lambda i: outDirName + "/%s.%04d.%s.tif" % (
        sampleName, sliceNumber, str(i))
    # run for every scan
    map(lambda i: processImage(imgdict[i], outNameFn(scanNameDict[i]), i),
        sorted(imgdict.keys()))
    print ('Slice %04d with offset %02.2f generated for scans:' % (
        sliceNumber, angOffset), imgdict.keys())
...
if sliceNumber >= 0:
    ...
    map(lambda cslice: generate_slices(scan_prefix, cslice),
        range(1, 2160, -1 * sliceNumber))
