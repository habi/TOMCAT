...
if sys.version[0:1] == "1":
    ...
    if python2 == "":
        ...
        os.system("xkbbell")
        os.system("xmessage -nearmouse -timeout 30 -buttons '' Python level "
                  "2 cannot be found")
        sys.exit(1)
...
try:
    ...
    os.system("xkbbell")
    os.system("xmessage -nearmouse -timeout 30 -buttons '' CaChannel module "
              "cannot be found")
    sys.exit(1)
...
Parser.add_option('-k', '--blocks', dest='blocks', type='int',
                  ...
                  metavar='1')
Parser.add_option('-c', '--correctionfactor', dest='correctionfactor',
                  type='float',
                  help='The correction / overlap factor to use for making '
                       'blocks from stack Default: %default',
                  default=1.02)
...
if (startpos is not None) & (endpos_ready):
    # number of lines
...
if endpos < startpos:
    ...
    # Bottom block ist visible in field of view when stample state is
    # highest in Y -> startpos must be less than endpos -> swap if this
    # condition is not met
    (endpos, startpos) = (startpos, endpos)

delta = endpos - startpos
# compute number of blocks needed to cover the distance between start and
# stop position; this is one larger than the number of full blocks fitting
# into delta.
# Federica, Jan 25 2008 - I think this number is twice the number of full
# blocks fitting into delta if delta cannot be divided exactly by the blocks
...
print "Total number of lines requested ..: " + str(
    numlines * nblocks - unusedlines)
print
...
while i < nblocks:
    ...
    if (CurrentStatus >= (CurrentStart - 0.05 * CurrentStart) and
        ...
        print "Waiting 60s for thermal compensation after the absorber has " \
              "been closed ...\n"
        time.sleep(60)
...
try:
    ...
    loglinetext = loglinetext + "Block number : " + str(i + 1) + "/" + \
        str(nblocks) + "\n"
    loglinetext = loglinetext + "Block size : " + str(blocksize) + "\n"
    loglinetext = loglinetext + "Number of lines in block : " + \
        str(numlines) + "\n"
    loglinetext = loglinetext + "Total number of scanned lines : " + \
        str(numlines * (i + 1)) + "\n"
    loglinetext = loglinetext + "Total number of lines requested : " +\
        str(numlines * (i + 1) - unusedlines) + "\n"
    loglinetext += "Scan end position at line : "
    if i + 1 == nblocks:
        loglinetext += str(numlines - unusedlines)
    loglinetext += "\n"
    loglinetext = "\n" + 60 * "-" + "\n"
