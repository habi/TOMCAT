...
if sys.version[0:1] == "1":
    ...
    if python2 == "":
        ...
        os.system("xkbbell")
        os.system(
            "xmessage -nearmouse -timeout 30 -buttons '' Python level 2 cannot be found")
        sys.exit(1)
...
try:
    ...
    os.system("xkbbell")
    os.system(
        "xmessage -nearmouse -timeout 30 -buttons '' CaChannel module cannot be found")
    sys.exit(1)
...
if path_arg_at:
    if len(sys.argv) - path_arg_at < 8:
        print "xx startpos, xx endpos, zz startpos, zz endpos," \
              ...
              "vertical lines must be given. -t is optional"
    show_help()
    sys.exit(1)
    ...
    try:
        ...
        try:
            ...
            print "Parameter " + str(
                path_arg_at) + ": xx Start Position is not a number; misstyped test flag?? "
            show_help()
            sys.exit(1)
            # xx endpos
    try:
        ...
        try:
            ...
            print "Parameter " + str(
                path_arg_at) + ": xx End Position is not a number; misstyped test flag?? "
            show_help()
            sys.exit(1)
    ...
    try:
        ...
        try:
            ...
            print "Parameter " + str(
                path_arg_at) + ": zz Start Position is not a number; misstyped test flag?? "
            show_help()
            sys.exit(1)
            # zz endpos
    try:
        ...
        try:
            ...
            print "Parameter " + str(
                path_arg_at) + ": zz End Position is not a number; misstyped test flag?? "
            show_help()
            sys.exit(1)
...
print "Start position for YLIN .....................: " + str(
    ystartpos) + " microns"
print "End position for YLIN .......................: " + str(
    yendpos) + " microns"
print "Start position for XXLIN ....................: " + str(
    xxstartpos) + " microns"
print "End position for XXLIN ......................: " + str(
    xxendpos) + " microns"
print "Start position for ZZLIN ....................: " + str(
    zzstartpos) + " microns"
print "End position for ZZLIN ......................: " + str(
    zzendpos) + " microns"

print "Total number of blocks to scan...............: " + str(
    nxxblocks * nzzblocks * nyblocks)

print "Number of XX-blocks .........................: " + str(nxxblocks)
print "size of the XX-block ........................: " + str(
    xxblocksize) + " microns"
print "Number of ZZ-blocks .........................: " + str(nzzblocks)
print "size of the ZZ-block ........................: " + str(
    zzblocksize) + " microns"
print "Number of Y-blocks ..........................: " + str(nyblocks)
print "size of the Y-block .........................: " + str(
    yblocksize) + " microns"
...
while i < nyblocks:
    ...
    while j < nzzblocks:
        ...
        while k < nxxblocks:
            ...
            if testonly != 1:
                CurrentStatus = chRingCurrent.getValCHK(
                    chRingCurrent.connected)
                AbsorberStatus = chAbsorberStatus.getValCHK(
                    chAbsorberStatus.connected)
                ...
                while (CurrentStatus <= (CurrentStart - 0.05 * CurrentStart)
                       or Interlock == 1 or AbsorberStatus == 0):
                    ...
                    if (CurrentStatus >= (CurrentStart - 0.05 * CurrentStart)
                            and Interlock == 0 and AbsorberStatus == 0):
                        print "\nThe absorber has been closed by the interlock!!!"
                        chAbsorberStatus.putValCHK(1,
                                                   chAbsorberStatus.connected)
                ...
                if beam_dump == 1:
                    ...
                    if abs(yposition - yactual) >= 5:  # more than 5 microns
                        ...
                        print "Move YLIN to position.............: " + str(
                            yposition)
                        chYLIN.putValCHK(yposition, chYLIN.connected)
                        print "Wait for Y motors ... "
                        moved1 = chYLINM1.getValCHK(chYLINM1.connected)
                    ...
                    zzactual = zzposition
                    if abs(zzposition - zzactual) >= 5:  # more than 5
                        # microns
                        print "Move ZZ to position.............: " + str(
                            zzposition)
                        chZZLIN.putValCHK(zzposition, chZZLIN.connected)
                        print "Wait for ZZ motors ... "
                        moved1 = chZZLIN.getValCHK(chZZLIN.connected)
            ...
            if previous == 0:
                ...
                newROI = "B" + str(
                    i * (nxxblocks * nzzblocks) + j * nxxblocks + k + 1)

                # newfileprefix=fileprefix+"X"+str( k + 1 )+"_Y"+str( i + 1 )+"_Z"+str( j +1) + "_"
                newfileprefix = fileprefix + "_B" + str(i * (nxxblocks *
                                                             nzzblocks) +
                                                        j * nxxblocks + k
                                                        + 1) + "_"
            else:
            ...
            if testonly == 1:
                ...
                print "Block xx position....................: " + str(
                    xxposition)
                print "Block zz position....................: " + str(
                    zzposition)
            ...
            while (CurrentStatus <= (CurrentStart - 0.05 * CurrentStart) or
                    Interlock == 1 or AbsorberStatus == 0):
                # while (CurrentStatus == (CurrentStart-0.05*CurrentStart) or Interlock==1 or AbsorberStatus==0):
                time.sleep(0.5)
                CurrentStatus = chRingCurrent.getValCHK(
                    chRingCurrent.connected)
                ...
                if (CurrentStatus >= (CurrentStart - 0.05 * CurrentStart) and
                        Interlock == 0 and AbsorberStatus == 0):
                    print "\nThe absorber has been closed by the interlock!!!"
            ...
            # Start tomoscan
            print "Acquiring tomo data for block " + str(
                i * nzzblocks * nxxblocks + j * nxxblocks + k + 1) + " over " + str(
                nyblocks * nzzblocks * nxxblocks) + "..."
            ...
            try:
                ...
                openlogfile = 1
                loglinetext = "\n------------------------------------------------------------\n"
                loglinetext = loglinetext + "Scan xx start position : " + str(
                    xxstartpos) + "\n"
                loglinetext = loglinetext + "Scan xx end position : " + str(
                    xxendpos) + "\n"
                loglinetext = loglinetext + "Scan zz start position : " + str(
                    zzstartpos) + "\n"
                loglinetext = loglinetext + "Scan zz end position : " + str(
                    zzendpos) + "\n"
                loglinetext = loglinetext + "Scan y start position : " + str(
                    ystartpos) + "\n"
                loglinetext = loglinetext + "Scan y end position : " + str(
                    yendpos) + "\n"

                loglinetext = loglinetext + "Block xx position : " + str(
                    xxposition) + "\n"
                loglinetext = loglinetext + "Block zz position : " + str(
                    zzposition) + "\n"
                loglinetext = loglinetext + "Block y position : " + str(
                    yposition) + "\n"

                loglinetext = loglinetext + "Block number : " + str(
                    i + 1) + "/" + str(nblocks) + "\n"

                loglinetext = loglinetext + "Block xx size : " + str(
                    xxblocksize) + "\n"
                loglinetext = loglinetext + "Block zz size : " + str(
                    zzblocksize) + "\n"
                loglinetext = loglinetext + "Block y size : " + str(
                    yblocksize) + "\n"
            ...
            if i * j * k == nxxblocks * nzzblocks * nyblocks - 1:
                ...
                AbsorberStatus = chAbsorberStatus.getValCHK(
                    chAbsorberStatus.connected)
                Interlock = chInterlock.getValCHK(chInterlock.connected)
                if (CurrentStatus <= (CurrentStart - 0.05 * CurrentStart) or
                        Interlock == 1 or AbsorberStatus == 0):
